...
- 内置壁纸：2025年12月 + 2026年全年（共3008张 HEIC，约561MB）
- 远程备份: `https://pub-810d6e0711de44d396071ecfc5ae9c2a.r2.dev`（PNG 格式）
- 加载顺序：Bundle → 磁盘缓存 → 网络下载
- 图片格式：内置使用 HEIC（压缩至原 PNG 的约 11%），远程使用 PNG
...
```
内置格式: {年后两位}.{月}.{日}.{语言}{方向}{序号}.heic
远程格式: {年后两位}.{月}.{日}.{语言}{方向}{序号}.png

示例:
  26.1.1.CS1.heic  → 1月1日 中文竖版 第1张（内置）
  26.12.25.EH2.png → 12月25日 英文横版 第2张（远程）
```

### 3.3 完整 URL 示例

```
https://pub-810d6e0711de44d396071ecfc5ae9c2a.r2.dev/wallpapers/26/01/26.1.1.CS1.png
```
...
| 2026-01-14 | v1.3.2 修复壁纸加载 | WallpaperViewModel 添加 @MainActor 确保主线程更新 |
